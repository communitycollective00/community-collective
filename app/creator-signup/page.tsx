...
const submit=async(e:FormEvent)=>{e.preventDefault(); const supabase=getSupabaseClient(); const {data,error}=await supabase.auth.signUp({email:form.email,password:form.password,options:{data:{full_name:form.fullName,professional_name:form.professionalName,username:form.username,industry:form.industry,bio:form.bio,role:"professional_pending",is_approved:false,invited:true}}}); if(error){setStatus(error.message);return;} try { if (data.user?.id) { await upsertProfileWithRetry(supabase,{id:data.user.id,email:form.email,fullName:form.fullName,username:form.username,role:"professional_pending"}); } setStatus("Account created. Pending approval."); } catch { setStatus("Account created. We're still finishing setup — please sign in in a moment."); }};
