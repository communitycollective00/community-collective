...
useEffect(()=>{if(!isAdmin) return; (async()=>{const {data}=await (getSupabaseClient().from("profiles") as any).select("id,full_name,email,role,industry,featured_requested").in("role",["pending_creator","verified_pending","professional_pending"]).order("created_at",{ascending:false}); setApps(data||[]);})();},[isAdmin]);
const act=async(id:string,action:"approve"|"decline"|"feature")=>{const patch=action==="approve"?{role:"professional",is_approved:true}:action==="feature"?{role:"professional",is_approved:true,featured:true}:{role:"public",is_approved:false,featured:false}; const {error}=await (getSupabaseClient().from("profiles") as any).update(patch).eq("id",id); setStatus(error?error.message:`${action}d`); setApps(apps.filter(a=>a.id!==id));};
